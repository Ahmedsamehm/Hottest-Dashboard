...
import { Button, buttonVariants } from "@/app/components/shared/ui//button";
...
function PaginationPrevious({ className, ...props }: React.ComponentProps<typeof PaginationLink>) {
  return (
    <PaginationLink aria-label="Go to previous page" size="default" className={cn("gap-1 px-2.5 sm:pl-2.5 disabled:opacity-50", className)} {...props}>
      <ChevronLeftIcon />
      <button disabled={props?.disabled} className="cursor-pointer  !hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50">
        Previous
      </button>
    </PaginationLink>
  );

...
import { Sidebar, SidebarMenuButton, SidebarGroup, SidebarGroupContent, SidebarHeader, SidebarMenuItem, SidebarContent, SidebarMenu, SidebarFooter } from "./shared/ui/sidebar";
import Link from "next/link";
import { Calendar, Bed, Hotel, Users, BarChart3 } from "lucide-react";
import { ModeToggle } from "./ModeToggle";
import { useGetUser, useLogout } from "../(Pages)/Login/_hooks/useLogin";
    ...
    <Sidebar className={`border-r border-gray-200 bg-background ${!user ? "invisible" : "visible"}`}>
      ...
      <SidebarHeader className="border-b border-gray-200 p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold ">HotelDash</h2>
      ...
      <SidebarContent className="p-4">
        <SidebarGroup>
          <div className="mb-4">
            <p className="text-xs font-medium  uppercase tracking-wider px-3">Navigation</p>
          </div>
          <SidebarGroupContent>
            <SidebarMenu className="space-y-1">
              {menuItems.map((item) => (
                <SidebarMenuItem key={item.id}>
                  <SidebarMenuButton asChild className="h-auto p-3  data-[active=true]:bg-blue-50 data-[active=true]:border-r-2 data-[active=true]:border-blue-600 rounded-lg">
      ...
      <SidebarFooter className="border-t border-gray-200 p-4">
        ...
        <Button disabled={isPending} onClick={handleLogout} className="mt-4 w-full">
          Logout
        </Button>
      </SidebarFooter>
    </Sidebar>
